...
(async () => {
  ...
  const target = await page.evaluate(async () => {
    ...
    function project(horizontal) {
      ...
      const scale = (2 * projectionScale) / denominator;
      const x = viewport.width / 2 + dot(vector, right) * scale;
      const y = viewport.height / 2 - dot(vector, up) * scale;
      if (x < 460 || x > 1040 || y < 80 || y > 740) return null;
      return { x, y, depth };
    }
    const now = new Date();
    const candidates = [];
  ...
  const labels = await page.locator(".fact-grid dt").allInnerTexts();
  const constellationFact = await page.locator("#ageFact").innerText();
  const distance = await page.locator("#distanceFact").innerText();
  const brightness = await page.locator("#brightnessFact").innerText();
  const direction = await page.locator("#directionFact").innerText();
  const colorFactCount = await page.locator("#colorFact").count();
  await page.screenshot({ path: "qa-atlas-click.png", fullPage: true });
  await browser.close();

  const ok = title && !title.startsWith("Gaia DR3") && title.includes(target.name) && summary.includes("자리") && summary.includes("별입니다") && !summary.includes("화면의 별 크기") && labels.join("|") === "별자리 표기|거리|밝기|지금 방향" && constellationFact.includes("자리") && constellationFact.includes("별") && distance.includes("광년") && brightness.includes("겉보기등급") && brightness.includes("절대등급") && direction.includes("고도") && colorFactCount === 0 && errors.length === 0;
  console.log(JSON.stringify({ ok, target, title, summary, labels, constellationFact, distance, brightness, direction, colorFactCount, errors }, null, 2));
