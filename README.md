...
- 실시간 별하늘 캔버스, 실제 밝은 별 5,046개, Stellarium western 별자리 선 88개
- 관측소 배경 ON/OFF와 선택 별이 구조물 뒤에 있을 때 자동 배경 투명화
- 별자리 선에 포함된 밝은 별 클릭 시 이름, 별자리, 거리, 밝기, 방향을 구분해 표시
- 친구 별 확대 애니메이션
...
- `data/sky-atlas.json`: D3-Celestial 밝은 별 좌표 5,044개에 Stellarium western 선 보강 별 2개를 더하고, Stellarium western 별자리 선 88개를 적용했습니다. HIP 번호로 SIMBAD/CDS 이름과 시차를 붙였고, 별자리 선에 포함된 691개 별은 클릭 대상입니다. 5,038개 별은 절대등급 계산이 가능합니다.
...
- `data/source/stellarium-western-index.json`: Stellarium western sky culture 별자리 선 원본.
- `data/source/constellations.lines.json`: D3-Celestial legacy 별자리 선 자료입니다. 현재 선 원본으로는 쓰지 않고, 재빌드 시 별자리 표시 우선순위(rank) 기준으로만 사용합니다.
...
SIMBAD/CDS 교차확인은 외부 서비스 요청이 많아서 시간이 걸릴 수 있습니다. 이미 생성된 JSON은 정적 파일이므로 GitHub Pages에 올려도 동작합니다. 절대등급은 별을 10파섹 거리에 둔다고 가정한 밝기이며, Gaia 검색 결과는 Gaia G 등급/거리, 별자리 클릭 결과는 V 등급/SIMBAD 시차로 계산합니다. 별자리 선은 Stellarium western HIP 연결 자료를 사용하고, thin 선 굵기 표식과 구면 위 짧은 호 보간을 반영해 화면에 그립니다.
...
- 별자리 선: [Stellarium western sky culture](https://github.com/Stellarium/stellarium-skycultures/blob/master/western/index.json), HIP 연결 자료
